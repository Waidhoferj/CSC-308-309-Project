...
def init_db():
    ...
    art = [
        ...
        Artwork(title="Hidden Subway Mural", description="Far side of the subway station has a double walled mural.", pictures=[get_sample_encoded_art_image()], location=[-120.67759172519696, 35.23446092387092], rating=89, metrics=ArtworkMetrics(total_visits=2000), comments=get_comments(12)),
        Artwork(title="Blue Bridge", description="Neon blue tentacles of paint wind up the struts of the bridge", pictures=[get_sample_encoded_art_image()], tags=["amazing"], location=[-120.70919388389524, 35.292870133451174], rating=90, metrics=ArtworkMetrics(total_visits=32), comments=get_comments(2)),
        Artwork(title="Artistic Underpass", description="Bridge ceiling covered in art", pictures=[get_sample_encoded_art_image()], tags=["surreal", "amazing"], location=[-120.69407509793804, 35.283794177611576], rating=97, metrics=ArtworkMetrics(total_visits=5127), comments=get_comments(7)),
        Artwork(title="Fire Wall", description="Tongues of flame comemorate the historic fire of this district", pictures=[get_sample_encoded_art_image()], tags=["sad", "historic", "amazing"], location=[-120.6354510797182, 35.28361728440618], rating=79, metrics=ArtworkMetrics(total_visits=7546), comments=get_comments(17)),
        Artwork(title="Street Corner Fresco", description="This popular street has a decorative fresco spilling out of the alley behind a popular restaurant", pictures=[get_sample_encoded_art_image()], tags=["amazing", "unreal"], location=[-120.71145568864382, 35.23659325190852], rating=82, metrics=ArtworkMetrics(total_visits=5234), comments=get_comments(1))
    ]
