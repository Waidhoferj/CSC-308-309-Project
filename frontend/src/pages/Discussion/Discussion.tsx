...
interface DiscussionProps {
  ...
  commentResolver: (fetchResult: any) => {
...
export default function Discussion(props: DiscussionProps) {
  const { loading, data, error, startPolling, stopPolling } = useQuery(
    props.fetchQuery,
    { variables: props.fetchVariables }
  );
  const { commentResolver } = props;
  const { comments, picture } = useMemo(
    () => commentResolver(data),
  ...
  const headerStyles = {
    background: picture
      ? "transparent"
      : "linear-gradient(90deg, #32a6ff70, transparent)",
  };
  ...
  function postMessage(message: string) {
    if (!profile) return;
    sendMessage({
      variables: props.postResolver({ message, author: profile.id }),
    });
  ...
  if (loading) return <Spinner absCenter={true} />;
    ...
    return (
      <ConnectionErrorMessage>
        Couldn't find the conversation.
      </ConnectionErrorMessage>
    );
  return (
